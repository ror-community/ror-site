...
In the future, we are going to be doing that slightly differently, because we will be implementing some processes to use the [ROR API](https://ror.readme.io/docs/api-about). I believe we're using the ROR API to update some institution data in our database with ROR data, but that is not propagating through our whole system as of right now. I didn't work on that project, that was one of our data engineers. But I believe at some point in the future, we will ping the ROR API once a day to check to see if any of the data has changed for specific institutions, and if there's a change, we will take that data and put it into our system.
