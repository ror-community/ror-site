...
Awesome. I'm going to take a bit of a side track and ask you what you think of the current discourse around AI, around artificial intelligence? 

### {{< figure src="/img/blog/openalex/justin_barrett.png" class="round-figure" alt="Justin Barrett" >}} Justin Barrett
 
Well, I don't think we're going to have problems yet. I think that it could be an issue in the future, and I think that the companies that are working on that now realize that. As far as AI being malicious, as far as AI taking our jobs, I think that AI is going to supplement our jobs for quite some time. People need to learn to use AI with their jobs,  because it can help you out. 

I use AI. I use ChatGPT to do a lot of different things, including just looking for ideas. I'll have conversations with ChatGPT and ask, "Hey, what can I do for this? What can I do for that?" just as a soundboard. But I think people need to accept that AI is going to be helping us with our jobs for quite some time, so just getting in front of that. 

I don't know. There's so many different issues that people are talking about with AI. As far as open source and AI, that's an interesting one, because that's something where you can get these very powerful models into the hands of people that might not use them the way we want them to. But that's something that I try not to think about too much.

### {{< figure src="/img/adam-sq-200.jpg" class="round-figure" alt="Adam Buttrick" >}} Adam Buttrick
 
We're actually using large language models now for a lot of different tasks at ROR, especially around the curation processing to do automatic triaging of requests. We have a pretty high [request volume](https://github.com/ror-community/ror-updates/issues) now, and it's immensely helpful to have a large language model come in and do a first pass and clean things up. We're doing it in exactly the way that you're talking about it: we're supplementing the existing work and then looking at other exploratory things. But it is controversial, especially in the world of scholarly communications.
 
Amanda and I are both based at [Crossref](https://crossref.org/people), and we're also looking at it for Crossref applications. At Crossref we're probably favoring trying to see where we can leverage open source models, just because it's an important part of Crossref's overall approach to make use of open source models versus tying everything to closed systems.

### {{< figure src="/img/blog/openalex/justin_barrett.png" class="round-figure" alt="Justin Barrett" >}} Justin Barrett
 
Yeah, I think that there are a lot of open source models coming out now that are showing a lot of promise. When ChatGPT first came out, there was no one that was even close to having that type of performance, and now you're starting to see the open source models catch up, which is very nice. 

I think as far as having one of those large language models in production, doing the things that I'm creating, like having an institution parser, I don't know that they're there yet. They're not fast enough. But when it comes to maybe creating a [training dataset](https://en.wikipedia.org/wiki/Training,_validation,_and_test_data_sets), that is something that they could really help with. 

All the training data we have had up until now has come straight from MAG, and the longer you go into MAG data, the more you see that it isn't the best. It is good enough to create these models, and you'll get pretty good performance. But there are enough errors where you can look at it and say, "Okay, maybe a large language model could give me some better training data or supplemental training data." That'll be interesting, and that's something we'll probably look into. 

### {{< figure src="/img/adam-sq-200.jpg" class="round-figure" alt="Adam Buttrick" >}} Adam Buttrick
 
We've been looking at doing a similar thing with improving our affiliation matching service, where we have a large language model come in and check concurrences between strategies. We throw an affiliation data set to three different strategies, and then see where there might be convergence or divergence on what's chosen as a ROR ID, and then have a large language model come through and do a pass. You can even make it explanatory if you want,  like have it say "We think it's this match or that match." That's something that we're looking at, because, as you said, they're not fast enough for doing that on their own, but for tasks that aren't really time-sensitive they can maybe provide some value.

### {{< figure src="/img/blog/openalex/justin_barrett.png" class="round-figure" alt="Justin Barrett" >}} Justin Barrett
 
Yeah, and even if it's for like a one-off task, like for affiliation matching, where you don't have agreement between the models, and it seems like it's a hard one, maybe you put that into a different queue, and that goes through a large language model to get that. So yeah, there are a lot of things that you can do with it. After this author disambiguation project is over, I think I'll have a lot more free time to explore some of those possibilities. But as of right now, it's just all author disambiguation for me right now.

### {{< figure src="/img/adam-sq-200.jpg" class="round-figure" alt="Adam Buttrick" >}} Adam Buttrick
 
Well, we'll stay tuned to all of your work, I'm sure.

### {{< figure src="/img/amanda-sq-200.png" class="round-figure" alt="Amanda French" >}} Amanda French

Moving on! When and where did you personally first hear about ROR?
...
Can you talk a little bit more about the streamlining? Why did you pivot to narrowing the [OpenAlex institution model](https://docs.openalex.org/api-entities/institutions) to use only ROR IDs?
...
That's great. But it also kind of highlights the need for publisher partners to contribute those, to impose that little bit of order onto the chaos of it and not let the data rot in the same way that the historic data did.
...
I have a related question. Did you work on the [new funder model](https://groups.google.com/g/openalex-users/c/NTTXy0315vg/m/Hg9XSgb5AQAJ) that has been integrated? 
...
I saw that the OpenAlex [funder model](https://docs.openalex.org/api-entities/funders) also integrated ROR, so I wondered if the ROR data was being pulled in in the same way.
...
It was very forward thinking to include ROR ID mappings, because over the long term, [the Funder Registry and ROR are intended to merge](https://crossref.atlassian.net/browse/CR-1208), essentially. It's a ways out, but you'll be ahead of the game, because you already have a Funder ID to ROR ID mapping in the Funder object, so you can just privilege one ID or the other.
...
Do you know if you'll leverage things like [ROR hierarchies and relationships](https://ror.readme.io/docs/ror-hierarchies-and-relationships) in the UI such that the works of child organizations can be associated with parent organizations? Some people have done that, and some people have not done that, so I wonder what OpenAlex's approach is.
...
Well, ROR is a confirmed API user of OpenAlex. Whenever someone [requests](https://curation-request.ror.org) to add an institution to ROR, the first thing we do is take the names and the aliases and ping them against the Crossref and OpenAlex APIs to see if there's affiliation usage for that institution in published works, because that's a [primary qualifying criterion for inclusion in ROR](https://ror.org/registry/#scope-and-criteria-for-inclusion). There's a little bot that comes through our GitHub issues and says, "Yes, all of these DOIs are associated with that institution." And that's based on the query results from the OpenAlex and the Crossref APIs. So we're a confirmed user, at least.

### {{< figure src="/img/blog/openalex/justin_barrett.png" class="round-figure" alt="Justin Barrett" >}} Justin Barrett
 
I see. So you're using bots on our API?

### {{< figure src="/img/adam-sq-200.jpg" class="round-figure" alt="Adam Buttrick" >}} Adam Buttrick
 
It's a script, and I say "Click"! 

### {{< figure src="/img/amanda-sq-200.png" class="round-figure" alt="Amanda French" >}} Amanda French
 
When you see those spikes in traffic, you know who to contact!

### {{< figure src="/img/adam-sq-200.jpg" class="round-figure" alt="Adam Buttrick" >}} Adam Buttrick
 
Yeah, the 500 requests that we send per month are gonna take down the OpenAlexAPI! For sure. 

That's not actually true. It's actually more than that, because, getting back to AI, one thing that we started doing is having a large language model generate potential aliases for a provided institution based on the information in a request. It tells us, "Here could be some potential aliases," and then we also ping those against the OpenAlex and Crossref APIs to identify affiliation usage. Sometimes when a curation request comes in, people put different versions of the organization's name, versions that aren't even used on the organization's website, or they don't include former names of the organization or things like that. Sometimes those are exposed by the LLM, and then we confirm that they're actually real and that they're valid based on affiliation usage. 
...
**ROR is playing a huge part in the author disambiguation as well, because we're using institutions to determine if authors are the same. Unfortunately, you can't only use institutions to decide that. You can't only use co-authors, you can't only use topics, so we're using a mixture of all of those things. We're getting that institution from the author's affiliation string and parsing it, and that's going directly into our author disambiguation to try and match that institution. So ROR, again, playing a vital part in our in our system.**
...
I know that the relationships between ROR institutions are very tricky sometimes. When I'm looking at the relationships I see sometimes an organization is a both a parent of another organization and also related laterally to it. I see a lot of weird relationships, and I don't really understand how they work, and maybe that is actually how they are. I think that the next step for us is to use that relationship data. I would hope that when I start using it, it's ready to go, but I don't know if that'll be the case. We are working right now to look at French institutions. I don't know if you are familiar with how complicated that is.

### {{< figure src="/img/adam-sq-200.jpg" class="round-figure" alt="Adam Buttrick" >}} Adam Buttrick
 
I'm working on a project with the [Ministry of Higher Education and Research](https://www.enseignementsup-recherche.gouv.fr/fr) in France, and it is very challenging to identify the relationships especially, as you say, and also what's an organization and what's not an organization. We're doing a lot of work on that. 
...
No, it's separate from HAL. It's called [RNSR](https://www.data.gouv.fr/en/datasets/repertoire-national-des-structures-de-recherche-rnsr/), the _Repertoire national des structures de recherche_. It enumerates all the French research structures which exist at various levels of solidity. 

We're working on a project to try to reflect all of the various French research units that operate semi-autonomously from their various parent organizations. They kind of function like independent research laboratories. We're starting out by going through and improving our records for what are called [UMRs, _Unités mixte de recherche_](https://en.wikipedia.org/wiki/Unit%C3%A9_mixte_de_recherche), or "mixed research units." We're making sure that they're better disambiguated and that they have the correct parents based on the RNSR data, and we're adding a bunch of them while also improving our coverage of French research organizations in general so that we have all of the parent organizations represented correctly. 

In general, we're doing a lot of work around improving French research organizations, and we are being actually [sponsored by the French government to do that](https://ror.org/blog/2023-07-24-community-supported-infra#supporting-ror-through-global-coalitions) through the [French Committee for Open Science](https://www.ouvrirlascience.fr/home/). Hopefully, that will accrue benefit to OpenAlex. But I'd also be interested to know what you find as a result of doing the analysis, because I'm sure there's things we've missed.
...
I did actually make [a very rough script](https://github.com/adambuttrick/fake_affiliations) to recreate what Justin did, to take the [ROR data dump](https://ror.readme.io/docs/data-dump) and convert it into a set of fake affiliation strings. I don't know if it exactly follows the patterns from the OpenAlex script, but others can do the same thing if they want to. If they're building their own models, they want to supplement, they want to train, they can just pull the ROR data, run it through the script and do that. 
...
I'm very excited about it, too, but I'm going to try to make it legible to a general audience like me, and that may take some work.
